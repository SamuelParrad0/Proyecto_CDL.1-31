      ...
      <section className="seccion-servicios" style={{ paddingTop: '120px' }}>
        ...
        <div className="servicios-cuadricula">
          {[
            ...
            <div key={s.titulo} className="tarjeta-servicio animar-entrada" style={s.delay ? { transitionDelay: s.delay } : {}}>
              ...
              <ul className="servicio-caracteristicas">
                {s.items.map((it) => <li key={`${s.titulo}-${it}`}><i className="fas fa-check"></i> {it}</li>)}
              </ul>
            </div>
          ))}
        </div>
