...
export default function CarritoScreen() {
  ...
  const [datosCheckout, setDatosCheckout] = useState({ direccion: '', telefono: '', notas: '' });
  const [datosPago, setDatosPago] = useState({ metodo: 'tarjeta', numero: '', titular: '', expiracion: '', cvv: '' });
  ...
  const [nuevaDirData, setNuevaDirData] = useState({
    ...
    barrio: '', apartCasa: '', telefono: '', indicaciones: '', residenciaLaboral: 'Residencia'
  });

  const confirmarVaciado = () => {
    Alert.alert('Vaciar Carrito', '¿Estás seguro de que deseas eliminar todos los productos?', [
      { text: 'Cancelar', style: 'cancel' },
      { text: 'Vaciar', style: 'destructive', onPress: () => vaciarCarrito() }
    ]);
  };

  const confirmarEliminacionItem = (item: any) => {
    Alert.alert('Cancelar Solicitud', `¿Estás seguro que deseas eliminar "${item.nombre}" del carrito?`, [
  ...
  const procesarCompra = () => {
    if (!estaAutenticado) {
      Alert.alert('Inicia Sesión', 'Necesitas iniciar sesión para completar tu compra.', [
        { text: 'Cancelar', style: 'cancel' },
  ...
  const guardarNuevaDireccion = async () => {
    const obligatorios = [nuevaDirData.nombreCompleto, nuevaDirData.direccion, nuevaDirData.departamento, nuevaDirData.municipio, nuevaDirData.barrio, nuevaDirData.telefono];
    ...
    try {
      setCargandoDirecciones(true);
      await servicioDirecciones.crearDireccion({
      ...
      const dirs = await servicioDirecciones.obtenerMisDirecciones();
      setDireccionesGuardadas(dirs);
      if (dirs.length > 0) setDireccionSeleccionada(dirs[dirs.length - 1]);
      setModoDireccion('seleccionar');
      setNuevaDirData({ nombreCompleto: '', direccion: '', departamento: '', municipio: '', barrio: '', apartCasa: '', telefono: '', indicaciones: '', residenciaLaboral: 'Residencia' });
  ...
  const handleConfirmarPedidoCheckout = async () => {
    if (datosPago.metodo === 'tarjeta') {
      const tarjetaIncompleta = !datosPago.numero.trim() || !datosPago.titular.trim() || !datosPago.expiracion.trim() || !datosPago.cvv.trim();
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: Espaciado.lg, paddingBottom: Espaciado.sm },
  titulo: { fontSize: 28, fontWeight: 'bold', color: Tema.dark.text },
  tituloDestacado: { color: Tema.dark.tint },
  subtitulo: { color: Tema.dark.textSecondary, fontSize: 14, marginTop: Espaciado.xs },
  botonVaciar: { flexDirection: 'row', alignItems: 'center', gap: 4, padding: Espaciado.sm },
  textoVaciar: { color: Tema.dark.error, fontSize: 14, fontWeight: '600' },
  cargandoContenedor: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  lista: { padding: Espaciado.lg, paddingTop: Espaciado.sm, gap: Espaciado.md },
  itemCarrito: { flexDirection: 'row', backgroundColor: '#16161E', borderRadius: RadioBorde.lg, padding: Espaciado.lg, borderWidth: 1, borderColor: '#2A2A35' },
  imagenPlaceholder: { width: 90, height: 90, borderRadius: RadioBorde.md, backgroundColor: '#1C1C26', overflow: 'hidden' },
  imagen: { width: '100%', height: '100%' },
  itemInfoContenedor: { flex: 1, flexDirection: 'column', marginLeft: Espaciado.md, justifyContent: 'space-between' },
  itemInfo: { marginBottom: Espaciado.sm },
  nombre: { color: '#fff', fontSize: 15, fontWeight: 'bold', marginBottom: 4 },
  precio: { color: Tema.dark.textSecondary, fontSize: 13, marginBottom: 4 },
  subtotalEtiqueta: { color: Tema.dark.tint, fontSize: 13, fontWeight: 'bold' },
  subtotalMonto: { color: Tema.dark.tint, fontSize: 15, fontWeight: 'bold' },
  controlesContenedor: { justifyContent: 'center', alignItems: 'flex-start' },
  botonCancelarSolicitud: { backgroundColor: 'transparent', borderWidth: 1, borderColor: Tema.dark.tint, paddingHorizontal: Espaciado.md, paddingVertical: 10, borderRadius: RadioBorde.md },
  textoCancelarSolicitud: { color: Tema.dark.tint, fontSize: 12, fontWeight: '600' },
  vacioContenedor: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: Espaciado.xl },
  vacioTitulo: { color: Tema.dark.text, fontSize: 20, fontWeight: 'bold', marginTop: Espaciado.lg, marginBottom: Espaciado.sm },
  vacioSubtitulo: { color: Tema.dark.textSecondary, textAlign: 'center', marginBottom: Espaciado.xl },
  botonExplorar: { backgroundColor: Tema.dark.tint, paddingHorizontal: Espaciado.xl, paddingVertical: Espaciado.md, borderRadius: RadioBorde.md },
  botonExplorarTexto: { color: '#fff', fontWeight: 'bold', letterSpacing: 2 },
  footer: { padding: Espaciado.lg, backgroundColor: Tema.dark.surface, borderTopWidth: 1, borderTopColor: Tema.dark.borderRed },
  totalContenedor: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: Espaciado.md },
  totalTexto: { color: Tema.dark.text, fontSize: 18, fontWeight: '600' },
  totalMonto: { color: Tema.dark.tint, fontSize: 26, fontWeight: 'bold' },
  botonComprar: { backgroundColor: Tema.dark.tint, alignItems: 'center', justifyContent: 'center', padding: Espaciado.lg, borderRadius: RadioBorde.md, width: '100%', marginTop: Espaciado.sm },
  botonComprarTexto: { color: '#fff', fontWeight: 'bold', fontSize: 16, letterSpacing: 2 },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(5, 5, 10, 0.85)', justifyContent: 'center', alignItems: 'center', padding: Espaciado.lg },
  modalContainer: { width: '100%', backgroundColor: '#0F0F14', borderRadius: RadioBorde.lg, borderWidth: 1, borderColor: '#2A2A35', overflow: 'hidden', position: 'relative', shadowColor: Tema.dark.tint, shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.3, shadowRadius: 30, elevation: 15 },
  closeButton: { position: 'absolute', top: 15, right: 15, width: 32, height: 32, borderRadius: 16, borderWidth: 1, borderColor: Tema.dark.tint, backgroundColor: 'rgba(211, 47, 47, 0.1)', justifyContent: 'center', alignItems: 'center', zIndex: 10 },
  modalScroll: { padding: Espaciado.xl, paddingTop: Espaciado.xxl + 10 },
  modalTitle: { color: '#fff', fontSize: 22, fontWeight: 'bold', textAlign: 'center', marginBottom: Espaciado.xs },
  modalSubtitle: { color: Tema.dark.textSecondary, fontSize: 14, textAlign: 'center', marginBottom: Espaciado.xl },
  inputGroup: { marginBottom: Espaciado.lg },
  label: { color: Tema.dark.textSecondary, fontSize: 11, fontWeight: 'bold', marginBottom: Espaciado.sm, letterSpacing: 1 },
  input: { backgroundColor: '#16161E', borderWidth: 1, borderColor: '#2A2A35', borderRadius: RadioBorde.md, padding: Espaciado.md, color: '#fff', fontSize: 15 },
  modalActions: { flexDirection: 'row', justifyContent: 'space-between', marginTop: Espaciado.md, gap: Espaciado.md },
  btnCancelarModal: { flex: 1, paddingVertical: Espaciado.md, borderRadius: RadioBorde.md, borderWidth: 1, borderColor: '#2A2A35', backgroundColor: '#16161E', alignItems: 'center' },
  btnCancelarModalTexto: { color: Tema.dark.textSecondary, fontSize: 14, fontWeight: '600' },
  btnConfirmarModal: { flex: 2, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', paddingVertical: Espaciado.md, borderRadius: RadioBorde.md, backgroundColor: Tema.dark.tint, gap: Espaciado.sm },
  btnConfirmarModalTexto: { color: '#fff', fontSize: 14, fontWeight: 'bold' },
  headerPasoVolver: { flexDirection: 'row', marginBottom: Espaciado.lg, alignItems: 'center' },
  btnVolverLinea: { flexDirection: 'row', alignItems: 'center', gap: 8, borderWidth: 1, borderColor: '#2A2A35', paddingHorizontal: Espaciado.md, paddingVertical: 6, borderRadius: RadioBorde.sm, backgroundColor: 'transparent' },
  textoVolver: { color: Tema.dark.tint, fontSize: 10, fontWeight: 'bold', letterSpacing: 1 },
  modalTitlePago: { color: '#fff', fontSize: 18, fontWeight: 'bold', marginBottom: Espaciado.lg, letterSpacing: 1 },
  metodosPagoColumna: { flexDirection: 'column', marginBottom: Espaciado.xl },
  metodosPagoFila: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10, width: '100%' },
  metodoPagoItem: { width: '48%', backgroundColor: '#16161E', borderWidth: 1, borderColor: '#2A2A35', borderRadius: RadioBorde.md, padding: Espaciado.lg, alignItems: 'center', justifyContent: 'center' },
  metodoPagoActivo: { borderColor: Tema.dark.tint, shadowColor: Tema.dark.tint, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.5, shadowRadius: 10, elevation: 5 },
  metodoPagoNombre: { color: '#fff', fontSize: 13, fontWeight: 'bold', marginTop: Espaciado.md, marginBottom: 4 },
  metodoPagoSub: { color: Tema.dark.textSecondary, fontSize: 10 },
  formularioTarjeta: { marginBottom: Espaciado.xl },
  labelSeccion: { color: Tema.dark.tint, fontSize: 14, fontWeight: 'bold', marginBottom: Espaciado.md, letterSpacing: 1 },
  filaInputs: { flexDirection: 'row', justifyContent: 'space-between' },
  resumenPedido: { backgroundColor: '#16161E', borderWidth: 1, borderColor: '#2A2A35', borderRadius: RadioBorde.lg, padding: Espaciado.lg, marginBottom: Espaciado.xl },
  resumenTitulo: { color: '#fff', fontSize: 14, fontWeight: 'bold', marginBottom: Espaciado.md, letterSpacing: 1 },
  resumenFila: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: Espaciado.sm },
  resumenItem: { color: '#fff', fontSize: 13 },
  resumenLabel: { color: Tema.dark.textSecondary, fontSize: 12, fontWeight: 'bold' },
  resumenValor: { color: '#fff', fontSize: 13, fontWeight: 'bold' },
  separadorResumen: { height: 1, backgroundColor: '#2A2A35', marginVertical: Espaciado.sm },
  resumenFilaTotal: { flexDirection: 'row', justifyContent: 'space-between', marginTop: Espaciado.xs },
  resumenTotalLabel: { color: '#fff', fontSize: 16, fontWeight: 'bold' },
  resumenTotalValor: { color: Tema.dark.tint, fontSize: 16, fontWeight: 'bold' },
  btnPagarFull: { backgroundColor: Tema.dark.tint, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', padding: Espaciado.lg, borderRadius: RadioBorde.md, gap: Espaciado.sm },
  btnPagarFullTexto: { color: '#fff', fontSize: 14, fontWeight: 'bold', letterSpacing: 1 },
  facturaContenedorPrincipal: { width: '100%' },
  facturaNotificacion: { borderWidth: 1, borderColor: '#00E676', borderRadius: RadioBorde.md, padding: Espaciado.lg, alignItems: 'center', marginBottom: Espaciado.lg, backgroundColor: 'rgba(0, 230, 118, 0.05)' },
  facturaNotiTitulo: { color: '#00E676', fontSize: 18, fontWeight: 'bold', marginTop: Espaciado.sm },
  facturaNotiSub: { color: Tema.dark.textSecondary, fontSize: 12 },
  facturaCard: { backgroundColor: '#fff', borderRadius: RadioBorde.md, padding: Espaciado.lg },
  facturaHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: Espaciado.lg, borderBottomWidth: 1, borderBottomColor: '#eee', paddingBottom: Espaciado.md },
  facturaLogoTexto: { color: Tema.dark.tint, fontStyle: 'italic', fontSize: 16, fontWeight: 'bold' },
  facturaInfoEmpresa: { color: '#666', fontSize: 10, marginTop: 2 },
  facturaHeaderDerecha: { alignItems: 'flex-end' },
  badgePagado: { backgroundColor: '#00E676', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4, marginBottom: 4 },
  badgePagadoTexto: { color: '#fff', fontSize: 9, fontWeight: 'bold' },
  facturaNumeroTexto: { color: Tema.dark.tint, fontSize: 12, fontWeight: 'bold', marginBottom: 4 },
  badgeConfirmado: { backgroundColor: '#00E676', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4 },
  badgeConfirmadoTexto: { color: '#fff', fontSize: 9, fontWeight: 'bold' },
  facturaSeccion: { marginBottom: Espaciado.md, borderLeftWidth: 2, borderLeftColor: Tema.dark.tint, paddingLeft: Espaciado.sm, backgroundColor: '#FAFAFA', padding: Espaciado.sm, borderRadius: 4 },
  facturaSeccionTitulo: { color: Tema.dark.tint, fontSize: 13, fontWeight: 'bold', marginBottom: Espaciado.sm },
  facturaFila: { flexDirection: 'row', marginBottom: 4 },
  facturaLabel: { width: 90, color: '#333', fontSize: 10, fontWeight: 'bold' },
  facturaValor: { flex: 1, color: '#666', fontSize: 10 },
  facturaTablaHeader: { flexDirection: 'row', justifyContent: 'space-between', backgroundColor: Tema.dark.tint, padding: 6, borderRadius: 4, marginBottom: Espaciado.sm },
  facturaTablaHeaderTexto: { color: '#fff', fontSize: 10, fontWeight: 'bold' },
  facturaTablaFila: { flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: '#eee', paddingVertical: Espaciado.sm },
  facturaTablaProducto: { color: '#333', fontSize: 11, fontWeight: '600' },
  facturaTablaPrecio: { color: Tema.dark.tint, fontSize: 11, fontWeight: 'bold' },
  facturaTotalesBox: { backgroundColor: '#F9F9F9', borderRadius: 8, padding: Espaciado.md, marginTop: Espaciado.md, alignSelf: 'flex-end', width: '70%', borderWidth: 1, borderColor: '#eee' },
  facturaFilaSubtotal: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 },
  facturaSubtotalLabel: { color: '#666', fontSize: 10 },
  facturaSubtotalValor: { color: '#333', fontSize: 10 },
  facturaTotalLinea: { height: 1, backgroundColor: Tema.dark.tint, marginVertical: Espaciado.sm },
  facturaTotalLabel: { color: Tema.dark.tint, fontSize: 12, fontWeight: 'bold' },
  facturaTotalValor: { color: Tema.dark.tint, fontSize: 12, fontWeight: 'bold' },
  facturaFilaMetodo: { flexDirection: 'row', justifyContent: 'space-between', borderTopWidth: 1, borderTopColor: '#eee', borderBottomWidth: 1, borderBottomColor: '#eee', paddingVertical: Espaciado.md, marginTop: Espaciado.md },
  facturaMetodoLabel: { color: '#333', fontSize: 11, fontWeight: 'bold' },
  facturaMetodoValor: { color: '#666', fontSize: 11 },
  facturaBotones: { flexDirection: 'row', justifyContent: 'space-between', marginTop: Espaciado.xl, gap: Espaciado.md },
  facturaBtnImprimir: { flex: 1, backgroundColor: Tema.dark.tint, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', padding: Espaciado.md, borderRadius: RadioBorde.sm, gap: 8 },
  facturaBtnImprimirTexto: { color: '#fff', fontSize: 12, fontWeight: 'bold' },
  facturaBtnNueva: { flex: 1, backgroundColor: '#fff', borderWidth: 1, borderColor: Tema.dark.tint, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', padding: Espaciado.md, borderRadius: RadioBorde.sm, gap: 8 },
  facturaBtnNuevaTexto: { color: Tema.dark.tint, fontSize: 12, fontWeight: 'bold' },
  direccionCard: { backgroundColor: '#16161E', borderWidth: 1, borderColor: '#2A2A35', borderRadius: RadioBorde.md, padding: Espaciado.md, marginBottom: Espaciado.sm },
  direccionCardActiva: { borderColor: Tema.dark.tint, backgroundColor: 'rgba(255, 8, 68, 0.05)' },
  direccionCardHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: Espaciado.xs, gap: 6 },
  direccionCardTipo: { color: Tema.dark.tint, fontSize: 11, fontWeight: 'bold', textTransform: 'uppercase' },
  direccionCardNombre: { color: '#fff', fontSize: 14, fontWeight: 'bold', marginBottom: 2 },
  direccionCardDir: { color: Tema.dark.textSecondary, fontSize: 13 },
  direccionCardLoc: { color: Tema.dark.textSecondary, fontSize: 12 },
  btnAgregarDireccion: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: Espaciado.md, borderWidth: 1, borderColor: Tema.dark.tint, borderRadius: RadioBorde.md, marginBottom: Espaciado.md, gap: 8 },
  btnAgregarDireccionTexto: { color: Tema.dark.tint, fontSize: 13, fontWeight: 'bold', letterSpacing: 1 },
  headerFormularioNuevaDir: { flexDirection: 'row', alignItems: 'center', marginBottom: Espaciado.lg, gap: 8, justifyContent: 'center' },
  modalTitleForm: { color: '#fff', fontSize: 18, fontWeight: 'bold', letterSpacing: 1 },
  inputGroupForm: { marginBottom: Espaciado.md },
  labelForm: { color: Tema.dark.textSecondary, fontSize: 10, fontWeight: 'bold', letterSpacing: 1, marginBottom: 4, textTransform: 'uppercase' },
  inputForm: { backgroundColor: '#16161E', borderWidth: 1, borderColor: '#2A2A35', borderRadius: RadioBorde.sm, color: '#fff', padding: Espaciado.sm, fontSize: 14 },
  rowForm: { flexDirection: 'row', justifyContent: 'space-between' },
  pickerContainerForm: { backgroundColor: '#16161E', borderWidth: 1, borderColor: '#2A2A35', borderRadius: RadioBorde.sm, overflow: 'hidden' },
  pickerForm: { color: '#fff', height: Platform.OS === 'ios' ? 120 : 45 },
  rowFormBotones: { flexDirection: 'row', justifyContent: 'space-between', gap: Espaciado.sm },
  btnTipoDomicilio: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: Espaciado.sm, borderWidth: 1, borderColor: '#2A2A35', borderRadius: RadioBorde.sm, gap: 6 },
  btnTipoDomicilioActivo: { borderColor: Tema.dark.tint, backgroundColor: Tema.dark.tint },
  btnTipoDomicilioTexto: { color: Tema.dark.textSecondary, fontSize: 11, fontWeight: 'bold', letterSpacing: 1 },
  btnGuardarForm: { flex: 2, backgroundColor: Tema.dark.tint, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: Espaciado.md, borderRadius: RadioBorde.md, gap: 8 },
  btnGuardarFormTexto: { color: '#fff', fontSize: 13, fontWeight: 'bold', letterSpacing: 1 },
  btnCancelarForm: { flex: 1, backgroundColor: '#16161E', borderWidth: 1, borderColor: '#2A2A35', alignItems: 'center', justifyContent: 'center', padding: Espaciado.md, borderRadius: RadioBorde.md },
  btnCancelarFormTexto: { color: Tema.dark.textSecondary, fontSize: 13, fontWeight: 'bold', letterSpacing: 1 },
});